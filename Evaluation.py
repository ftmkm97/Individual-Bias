...
def run(Database,Method,df):
    all_labels = np.array(df['AcceptedAnswer']) 
    all_preds = np.array(df['Score'])
    all_qid = np.array(df['QuestionId'])

    all_labels, all_preds = group_resuts(all_labels, all_preds, all_qid)

    res = cal_metric(all_labels, all_preds, ['mean_mrr', 'P@1;3;5;10', 'ndcg@5;10'])

    df = pd.read_csv('Results/Evaluation.csv')
    res['Dataset'] = Database
    res['Method'] = Method
    df = df._append(res,ignore_index=True)
    df.to_csv('Results/Evaluation.csv',index=False)


    return print('Evaluation Process is completed.')